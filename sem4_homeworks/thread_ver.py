...
download_dir = Path(Path.cwd(), 'download', 'thread')
...
def main(urls):
    threads = []
    for url in urls:
        thread = threading.Thread(target=download_image, args=(url.strip(),))
    ...
    print(f'Download complete! Total time for {len(threads)} equal: {time() - start_time:0.2f} seconds')
